...
internal partial class ImportBlingoFilesStep : VBoxContainer
{
    ...
    public ImportBlingoFilesStep(BlingoProjectSettings settings)
    {
        _settings = settings;
        Visible = false;
        ...
        _bulkType.AddItem("Parent", (int)BlingoScriptType.Parent);
        _bulkType.AddItem("Movie", (int)BlingoScriptType.Movie);
        _bulkType.AddItem("Behavior", (int)BlingoScriptType.Behavior);
    ...
    private void LoadFolder(string path)
    {
        ...
        foreach (var file in Directory.GetFiles(path, "*.ls"))
        {
            ...
            opt.AddItem("Parent", (int)BlingoScriptType.Parent);
            opt.AddItem("Movie", (int)BlingoScriptType.Movie);
            opt.AddItem("Behavior", (int)BlingoScriptType.Behavior);
            opt.Selected = 2;
            row.AddChild(opt);
            row.SetMeta("opt", opt);
    ...
    private void OnImportPressed()
    {
        ...
        var scripts = new List<BlingoScriptFile>();
        foreach (var child in _fileList.GetChildren())
        {
            if (child is HBoxContainer row)
            {
                ...
                var type = (BlingoScriptType)opt.GetSelectedId();
                scripts.Add(new BlingoScriptFile(Path.GetFileNameWithoutExtension(path),File.ReadAllText(path),type));
            }
        }
        ...
        foreach (var script in scripts)
        {
            ...
            {
                BlingoScriptType.Movie => Path.Combine(_settings.ProjectFolder, "MovieScripts"),
                BlingoScriptType.Parent => Path.Combine(_settings.ProjectFolder, "ParentScripts"),
                BlingoScriptType.Behavior => Path.Combine(_settings.ProjectFolder, "Sprites", "Behaviors"),
