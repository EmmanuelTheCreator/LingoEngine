...
{
    internal class BlLegacyCastItemReader_ForDump : BlLegacyCastItemReader_Dir10
    {
        ...
        #region For dump diagnose

        public (List<CastToken> Tokens, BlCastRawMemberItem? MemberItem) ReadItemWithTokensForDump(string name, byte[] info)
        {
            ...
            // Read the offsets
            (bool hasName, List<int> offsets) = ReadOffsetsForDump(returnData, reader, (int)typeValue, (int)infoLength, (int)specificLength);
            ...
            // Create slices
            var bytesValues = SliceOffsetRanges_ForDump(endingBytes, offsets, 0);
            ...
            // read "N/A"
            var userName = bytesValues[readOffsetLast3 + 2].ReadCString(0);


            //var text1 = TokenListToStringX(returnData);

            BlCastRawMemberItem? castMember = CreateMember_Dir10(infoSlice, specificData, memberName, memberContentType, memberContentType, blobs, dateCreated, dateModified, new List<int>(), userName,"");
            return (returnData, castMember);
        }

        private static (bool HasName, List<int> Offsets) ReadOffsetsForDump(List<CastToken> returnData, BlStreamReader reader, int typeValue, int infoLength, int specificLength)
        {
        ...
        private static List<byte[]> SliceOffsetRanges_ForDump(byte[] data, List<int> offsets, int startIndex = 5, int? endIndex = null)
        {
