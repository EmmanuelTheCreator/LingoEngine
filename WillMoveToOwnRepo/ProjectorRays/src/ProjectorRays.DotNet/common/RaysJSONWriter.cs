...
public class RaysJSONWriter : RaysCodeWriter
{
    ...
    public RaysJSONWriter(string lineEnding, string indentation = "  ")
    ...
    {
        WriteValuePrefix();
        Write(RaysUtil.FloatToString(val));
        _context = Context.Value;
        WriteValueSuffix();
    }
    ...
    public void WriteFourCC(uint val)
    {
        WriteValuePrefix();
        Write("\"");
        Write(RaysUtil.FourCCToString(val));
        Write("\"");
        _context = Context.Value;
        WriteValueSuffix();
    }
    ...
    {
        Write("\"");
        Write(RaysUtil.EscapeString(str));
        Write("\"");
    }
