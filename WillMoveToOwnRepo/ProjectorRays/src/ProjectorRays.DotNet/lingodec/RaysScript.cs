...
public class RaysScript
{
    ...
    public List<RaysHandler> Handlers = new();
    public List<LiteralStore> Literals = new();
    public List<RaysScript> Factories = new();

    public uint Version;
    public ScriptContext? Context;

    public RaysScript(uint version)
    {
        Version = version;
    }
    ...
    public void Read(ReadStream stream)
    {
        ...
        for (int i = 0; i < HandlersCount; i++)
            Handlers.Add(new RaysHandler(this));
    ...
    public void WriteVarDeclarations(RaysCodeWriter code)
    {
        if (!IsFactory())
        {
    ...
    /// </summary>
    public void WriteScriptText(RaysCodeWriter code, bool dotSyntax)
    {
        int orig = code.Size;
        WriteVarDeclarations(code);
        if (IsFactory())
    ...
    {
        var cw = new RaysCodeWriter(lineEnding);
